...
static void optee_supp_send(struct optee *optee,
			const struct teesmc_arg *arg,
			struct teesmc_arg *resp)
{
...
void copy_back_outdata(struct teesmc_arg *arg,
			const struct teesmc_arg *resp)
{
	struct teesmc_param *arg_params = TEESMC_GET_PARAMS(arg);
	struct teesmc_param *resp_params = TEESMC_GET_PARAMS(resp);
	size_t n;

	/* Copy back out and inout parameters */
	for (n = 0; n < arg->num_params; n++) {
		struct teesmc_param *ap = arg_params + n;
...
void optee_supp_thrd_req(struct tee_context *ctx, struct teesmc_arg *arg)
{
	...
	const size_t s = TEESMC_GET_ARG_SIZE(OPTEE_RPC_NUM_BUFS);
	struct teesmc_arg *resp;

	if (arg->num_params != OPTEE_RPC_NUM_BUFS) {
...
static u32 memref_to_user(struct tee_shm *shm,
			struct teesmc_param_memref *ph_mem,
			struct teesmc_param_memref *user_mem, int *fd)
{
	int res;
	phys_addr_t pa;
...
static int optee_supp_copy_to_user(void __user *buf,
			const struct teesmc_arg *arg, struct teesmc_arg *tmp)
{
	size_t s = TEESMC_GET_ARG_SIZE(OPTEE_RPC_NUM_BUFS);
	struct teesmc_param *arg_params;
	struct teesmc_param *tmp_params;
	size_t n;
	int ret;

	memcpy(tmp, arg, s);
	arg_params = TEESMC_GET_PARAMS(arg);
	tmp_params = TEESMC_GET_PARAMS(tmp);
